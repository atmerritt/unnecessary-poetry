...
swaggerui_blueprint = get_swaggerui_blueprint(
    ...
    API_URL,
    config={
        'app_name': "Unnecessary Poetry API"
    }
)
...
def validate_chat_request(data):
    """Validates and sanitizes API poem request data"""
    ...
    if len(text) > 4000:  # Reasonable limit
        return False, "Text is too long (max 4000 characters)"
...
def call_llm_api(user_input):
    ...
    prompt = """
    You are a creative poet who likes to take inspiration from random people,
    creatures, activies, objects, etc that you encounter during your everyday life.

    Please provide a poem in whatever style you prefer, based on the user input.

    The poem should be very short, no more than 8 lines.
    """
    chat_request = {
